...
u32 BigFontInit(CreditsTextProc * proc, signed char * str, int rowID)
{
    ...
    while (*str != 0)
    {
        ...
        /* JESTER - Apparently this comparison is always false due to the limited range of the data type */
...
// Replicate the SET_DATA macro functionality
__asm__(".global sCGDataTable\n"
        ".type sCGDataTable, object\n"
        ".set sCGDataTable, 0x80b65F0\n");
...
__asm__(".global sConvoBackgroundData\n"
        ".type sConvoBackgroundData, object\n"
        ".set sConvoBackgroundData, 0x800E894\n");
