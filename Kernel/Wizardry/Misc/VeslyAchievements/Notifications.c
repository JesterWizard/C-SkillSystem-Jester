...
{
    if (CheckInSaveScreen() || Proc_Find(ProcScr_WorldMapMain))
    {
        return 0x200;
    }
    if (CheckInBattleAnim())
...
void NotificationWindow_LoopDrawText(struct NotificationWindowProc * proc)
{
    ...
    // if (!proc->showingBgm) // for testing
    // {
...
char * NotificationPrintText(struct NotificationWindowProc * proc, struct Text * th, const char * str, int maxWidth)
{
    ...
    while (*iter > CHAR_NEWLINE)
    {
        ...
        iter = (void *)Text_DrawCharacter(th, (void *)iter);
        break;
    }
...
void NotificationWindowClean(struct NotificationWindowProc * proc)
{
    ...
    if (proc->spriteText)
    {
        ...
        if (CanNotifsDisplayCurrently(proc, Proc_Find(gProcScr_PlayerPhase)))
        {
            BlendSprites(0);
        }
    }
