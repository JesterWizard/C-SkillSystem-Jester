...
struct ProcCmd const gProcScr_NotificationWindow[] = {
    ...
    PROC_WHILE_EXISTS(ProcScr_CamMove), // ProcScr_CamMove
...
/* JESTER - Better to keep this off for now. Nothing here needs it and it fails to show the terrain name */
//! FE8U = 0x0808CB34
// LYN_REPLACE_CHECK(TerrainDisplay_Init);
...
{
    int i = 0;
    struct SoundRoomEnt const * data = gSoundRoomTable_NEW;
    if (data[id - 1].bgmId == id)
...
const char * GetPlayingBGMName(struct NotificationWindowProc * proc)
{
    ...
    CopyString(buf, "Bgm: ");
    GetStringFromIndexInBuffer(data[id].nameTextId, buf + strlen(buf));
