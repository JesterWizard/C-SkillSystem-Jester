...
int SpdGetterSkills(int status, struct Unit *unit)
{
    ...
    if (SkillTester(unit, SID_TakerSpeed) && gpKernelDesignerConfig->reset_bwl_stats_each_chapter == true)
    {
    ...
    if (SkillTester(unit, SID_Freelancer) && UNIT_WRANK(unit, ITYPE_SWORD) >= 1)
