...
int ResGetterSkills(int status, struct Unit *unit)
{
    ...
    if (SkillTester(unit, SID_TakerResistance) && gpKernelDesignerConfig->reset_bwl_stats_each_chapter == true)
    {
...
int ResPsychUpCheck(int status, struct Unit *unit)
{
    ...
    {
        stolen_status = ResGetterWeaponBonus(0, GetUnit(gBattleTarget.unit.index)) + 
                        ResGetterSkills(0, GetUnit(gBattleTarget.unit.index)) +
                        GetStatDebuffMsgBuf(GetUnit(gBattleTarget.unit.index))->res;
