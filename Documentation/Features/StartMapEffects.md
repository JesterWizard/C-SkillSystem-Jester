...
| Effect table | [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Stores each effect's label, R text, behavior kind, numeric value, duration, and target faction. |
| Menu setup | `StartMapEffectsPrompt_OnInit` in [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Initializes text, UI graphics, cursor hand, and the hidden-unit state while the prompt is open. |
| List rendering | `StartMapEffectsPrompt_Draw` in [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Draws the visible list, the scroll bar, the cursor hand, and the selected R text. |
| Menu teardown | `StartMapEffectsPrompt_Finish` in [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Restores the sprite visibility, clears the menu area, and ends the prompt cleanly. |
| Effect application | `StartMapEffects_ApplyStatEffect` and `StartMapEffects_ApplyMovEffect` in [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Applies the selected effect to matching units during stat and movement reads. |
| Pre-phase hook | `StartMapEffects_PrePhaseHook` in [`StartMapEffects.c`](../../Kernel/Wizardry/StartMapEffects/Source/StartMapEffects.c) | Starts the prompt on the first entry, then counts down the effect duration on later turns when the feature is enabled. |
