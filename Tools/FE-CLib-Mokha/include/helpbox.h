...
void sub_80898C4(void* vram, int palId); // ??? sub_80898C4(???);
...
// ??? HelpBoxDrawPrefix(???);
void HelpBoxIntroDrawTexts(struct ProcHelpBoxIntro * proc); // ??? HelpBoxIntroDrawTexts(???);
...
u16 GetDialogueBoxConfig(void); // ??? GetDialogueBoxConfig(???);
// ??? sub_808A530(???);
void InitBoxDialogue(void * vram_dst, int pad_idx); // ??? InitBoxDialogue(???);
// ??? sub_808A7B0(???);
// ??? SetBoxDialogueSize(???);
// ??? sub_808A848(???);
// ??? sub_808A87C(???);
